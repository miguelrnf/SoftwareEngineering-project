    ...
    <v-card class="mx-auto" max-height="80%">
      ...
      <div v-if="questions.length !== 0">
        <div v-for="item in questions" :key="item.content">
          <v-card flat>
            ...
            <div class="overflow-hidden">
              <v-bottom-navigation :input-value="showNav">
        ...
        <div class="text-center mb-2">
          <v-btn text color="primary" @click="showNav = !showNav" small>
            Show Answers
          </v-btn>
        </div>
      </div>
      ...
      <div v-else>
        ...
        <v-btn text small color="primary" @click="createQuiz(topic.name)"
...
export default class ShowSummaryDialog extends Vue {
  ...
  @Prop({ type: StatementManager, required: true })
  readonly statementManager!: StatementManager;
  questions: Question[] = [];
  ...
  getCorrect(options: Option[]) {
    for (let i = 0; i < options.length; i++) {
      if (options[i].correct) return options[i].content;
    }
  }
