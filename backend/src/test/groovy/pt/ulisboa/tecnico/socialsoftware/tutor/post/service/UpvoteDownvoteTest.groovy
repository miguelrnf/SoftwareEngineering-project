...
@DataJpaTest
class UpvoteDownvoteTest extends Specification{
    ...
    def "valid upvote"() {
        when:
        def upvotes = VALID_P.getUsersWhoUpvoted().size()
    ...
    def "valid downvote"() {
        when:
        def downvotes = VALID_P.getUsersWhoDownvoted().size()
    ...
    def "change downvote to upvote"() {
        when:
        def result1 = postService.vote(VALID_P.getKey() as Integer, VALID_U as User, "downvote")
