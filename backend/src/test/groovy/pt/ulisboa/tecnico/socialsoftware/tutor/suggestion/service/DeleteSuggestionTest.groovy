...
class DeleteSuggestionTest extends Specification{
    ...
    def "valid deletion"() {
        ...
        println('*' * 100)
        println(suggestionRepository.findAll().dump())
        println('*' * 100)
        suggestionService.deleteSuggestion(courseExecution.getId(), sug as SuggestionDto, new UserDto(VALID_U as User))
        println('*' * 100)
        println(suggestionRepository.findAll().dump())
        println('*' * 100)

        then:
        suggestionRepository.findByKey(sug.getKey()).get() == null
